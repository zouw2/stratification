...
library(knitr)
    ...
    asl <- as.data.frame( read_sas('/ocean/harbour/CDT70424/GO44010/DataReviewMonComm_IMC_6th/prod_imc6th_v1/data/adam/adsl.sas7bdat') )
   # 
#    raw <- rice_read_zip('root/clinical_studies/RO7496353/CDT70424/GO44010/source/other/prod/GO44010_Daily_Extract_20240429125425.zip')
   # 
rs <- as.data.frame( read_sas("/ocean/harbour/CDT70424/GO44010/DataReviewMonComm_IMC_6th/prod_imc6th_v1/data/sdtmv/rs.sas7bdat"))

tu <- as.data.frame( read_sas("/ocean/harbour/CDT70424/GO44010/DataReviewMonComm_IMC_6th/prod_imc6th_v1/data/sdtmv/tu.sas7bdat"))

tr <- as.data.frame( read_sas("/ocean/harbour/CDT70424/GO44010/DataReviewMonComm_IMC_6th/prod_imc6th_v1/data/sdtmv/tr.sas7bdat"))

  
outF <- '~/Migration/GO44010/results'

rrsp1 <- as.data.frame( read_sas('/ocean/harbour/CDT70424/GO44010/DataReviewMonComm_IMC_6th/prod_imc6th_v1/data/other/rrsp1.sas7bdat'))

analysis <- 'mar_2025'
```

this feature from [the RECIST FAQ](https://recist.eortc.org/faq/the-definition-of-sd-a-clarification/#:~:text=In%20the%20main%20article%20on,sum%20diameters%20while%20on%20study.%E2%80%9D) was not implemented: 
...
# for unscheduled visit,   for each record in ds1 will be forced to match a record in ds2, even their dates are very different
    ...
    if(!any(c(uns1, uns2))) return (list( ds12 ) )
     ...
     #  
      ...
      for (i in 1:nrow(ds1b)) {
        ...
        # so for each record in ds1, the program will always try to find a record in ds2 that can match.
        # a logic hole is that this may be not true, only a portion of record in ds1 that can be matched.
...
write.csv(patient_tu, file= paste(outF,'/patient_tu', analysis,'.csv', sep = ''), row.names=F)

get_tumor_list <- function(d) {
   dlply(d, .(USUBJID), function(d1) {
     u1 <- unique(d1$TULNKID)
     stopifnot(length(u1) > 0)
     u1
     })
}
...
# sld_100028 <- subset(sld, USUBJID %in% 'GO44010-100028')
# sld_100040 <- subset(sld, USUBJID %in% 'GO44010-100040')
# sld_100077 <- subset(sld, USUBJID %in% 'GO44010-100077')
...
diff_target <- merge(asl[, c('USUBJID','ACTARM')], diff_target, by='USUBJID')

write.csv(diff_target[, c('USUBJID','ACTARM','visit2','RSSTRESC', 'sld','early_date_target',	'late_date_target',	'base',	'nadia',	'chg_base',	'chg_nadia',	'targetResp')]
, file = paste(outF,'/diff_target_', analysis,'.csv', sep = ''), row.names=F)
...
diff_nt <- merge(asl[, c('USUBJID','ACTARM')], diff_nt, by='USUBJID')


write.csv(diff_nt[, c('USUBJID','ACTARM','visit2','RSSTRESC', 'early_date_nt',	'late_date_nt',	'ntRespVisit')]
, file = paste(outF,'/diff_nontarget_', analysis,'.csv', sep= ''), row.names=F)
...
known bugs: when merging with new lesion data and there are unscheduled visits in both target/no target data and new lesion data, each new lesion record from unscheduled visit will be merged to all unscheduled visits from target/non target data. this cause a wrong report for GO44010-100028, UNSCHEDULED 2024-01-18
...
resp2 <- merge(asl[, c('USUBJID','ACTARM')], resp2, by='USUBJID')


  
  write.csv( subset(resp2, overall!= OVRLRESP), file =   paste(outF, '/diff_overall_', analysis,'.csv', sep = ''), row.names=F)
